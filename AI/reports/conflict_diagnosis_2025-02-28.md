...
- **Analysis**: This branch attempts to add Cloudflare Vectorize types (`VectorizeIndex`) and other changes _inside_ the legacy structure (or unrelated root).
...
2.  **Create a New Branch**:
    ...
    ```
3.  **Port Changes**: Manually copy _only_ the unique source files (e.g., new components, logic) from your old branch to the new structure in `apps/gs-web/`, `apps/gs-admin/`, etc.
    - _Tip:_ Use `git show origin/old-branch:path/to/file > path/to/new-file` to retrieve content.
