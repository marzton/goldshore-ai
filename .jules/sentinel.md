...
**Vulnerability:** The shared `verifyAccess` utility checked the Token Issuer but ignored the Audience claim. This allowed a valid Cloudflare Access token from _any_ application in the GoldShore organization to authenticate against _any other_ internal service using this library.
...
## 2026-02-15 - Exposed Client Secret via PUBLIC\_ Env Var
...
**Vulnerability:** The contact form API (`apps/web/src/pages/api/contact.ts`) was vulnerable to HTML injection in auto-responder emails. An initial fix attempted to sanitize input at the API layer _and_ the email template layer.
