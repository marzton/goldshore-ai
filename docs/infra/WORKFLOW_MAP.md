# Workflow Map & Inventory (Phase 0)

This document maps all GitHub Actions workflows in the repository, their triggers, dependencies, and current status.
Generated by Jules (Phase 0: Containment).

## Workflow Inventory

| Workflow Name | File Path | Trigger | Dependencies | Reusable Refs | Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Deploy GS Agent | `.github/workflows/deploy-gs-agent.yml.disabled` | `push` (main) | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | **DISABLED** | Explicitly disabled during Phase 0. |
| Deploy GS API | `.github/workflows/deploy-gs-api.yml` | `push` (main) | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Deploys using Wrangler. |
| Deploy GS Control | `.github/workflows/deploy-gs-control.yml.disabled` | `push` (main) | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | **DISABLED** | Explicitly disabled during Phase 0. |
| Deploy GS Gateway | `.github/workflows/deploy-gs-gateway.yml.disabled` | `push` (main) | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | **DISABLED** | Explicitly disabled during Phase 0. |
| Deploy GS Mail | `.github/workflows/deploy-gs-mail.yml` | `push` (main) | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Deploys using Wrangler. |
| Jules Nightly | `.github/workflows/jules-nightly.yml` | `schedule`, `workflow_dispatch` | None | `jules-daily.yml`, `jules-nightly-clean.yml`, `palette-daily.yml`, `sentinel-nightly.yml`, `conflict-sweeper-nightly.yml` | **INVALID** | References missing reusable workflows. Requires repair or removal. |
| Lockfile Guard | `.github/workflows/lockfile-guard.yml` | `workflow_dispatch` | `actions/checkout` | None | **MANUAL** | Enforces strict lockfile policy. Disabled automation (Cost Containment). |
| Deploy Frontend Pages | `.github/workflows/deploy-frontend.yml` | `push` (main), `workflow_dispatch` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node`, `cloudflare/pages-action` | None | VALID | Deploys gs-web and gs-admin in parallel from the same commit SHA. |
| Maintenance: Agent Sync Audit | `.github/workflows/maintenance-agent-sync.yml` | `workflow_dispatch` | `actions/checkout`, `actions/setup-node` | None | VALID | Manual repo audit, optional builds/tests, and gs service-token sync checks. |
| Maintenance: Branch Report | `.github/workflows/maintenance.yml` | `schedule`, `workflow_dispatch` | `actions/checkout`, `actions/upload-artifact` | None | VALID | Inspection-only branch reporting with a read-only artifact upload. |
| Naming Guard | `.github/workflows/naming-guard.yml` | `workflow_dispatch` | `actions/checkout` | None | **MANUAL** | Checks for legacy paths/files. Disabled automation (Cost Containment). |
| Naming Lint | `.github/workflows/naming-lint.yml` | `pull_request`, `push` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Runs `pnpm check:naming`. |
| NeuraLegion Scan | `.github/workflows/neuralegion.yml` | `schedule`, `workflow_dispatch` | `actions/checkout`, `NeuraLegion/run-scan` | None | **VALID** | Now uses `ubuntu-latest` runner. |
| Palette Dispatch Run | `.github/workflows/palette-manual.yml` | `repository_dispatch` | `actions/checkout`, `actions/setup-node` | None | VALID | Dispatch-triggered Palette agent run. |
| PII Scan | `.github/workflows/pii-scan.yml` | `workflow_dispatch` | `actions/checkout`, `actions/setup-node`, `actions/upload-artifact` | None | **MANUAL** | Runs `pnpm scan:pii`. Disabled automation (Cost Containment). |
| Preview Agent | `.github/workflows/preview-gs-agent.yml` | `pull_request` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Deploys preview using Wrangler. |
| Preview GS Admin | `.github/workflows/preview-gs-admin.yml` | `pull_request` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node`, `cloudflare/pages-action` | None | VALID | Deploys preview to Cloudflare Pages. |
| Preview GS API | `.github/workflows/preview-gs-api.yml` | `pull_request` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Deploys preview using Wrangler. |
| Preview GS Gateway | `.github/workflows/preview-gs-gateway.yml` | `pull_request` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Deploys preview using Wrangler. |
| Preview GS Web | `.github/workflows/preview-gs-web.yml` | `pull_request` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node`, `cloudflare/pages-action` | None | VALID | Deploys preview to Cloudflare Pages. |
| Route Collision Check | `.github/workflows/route-collision-check.yml` | `pull_request`, `push` | `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` | None | VALID | Runs route collision check. |
| SonarCloud | `.github/workflows/sonarcloud.yml` | `push`, `pull_request` | `SonarSource/sonarcloud-github-action` | None | VALID | Uses SonarCloud action. |
| Stabilization Task | `.github/workflows/stabilization-task.yml` | `schedule`, `workflow_dispatch` | `actions/checkout`, `actions/setup-node`, `stefanzweifel/git-auto-commit-action` | None | VALID | Runs daily stabilization checks. |
| Summary | `.github/workflows/summary.yml` | `issues` | `actions/checkout`, `actions/ai-inference` | None | **SUSPICIOUS** | Uses `actions/ai-inference@v2` which may be invalid or costly. |
| TFSec | `.github/workflows/tfsec.yml` | `schedule` | `actions/checkout`, `aquasecurity/tfsec-sarif-action`, `github/codeql-action` | None | VALID | Runs security scan. |

## Cross-Repo References

No cross-repository workflow triggers (e.g., `repository_dispatch` targeting other repos) were identified.
External actions are pinned or versioned appropriately, except for:
*   `NeuraLegion/run-scan`: Uses specific SHA.
*   `actions/ai-inference`: Uses `v2`. Verify origin/validity.

## Governance Observations

1.  **Broken Governance Workflows**: The `jules-nightly.yml` workflow is completely non-functional as it relies on missing files.
2.  **Deprecated Runners**: `neuralegion.yml` runner has been updated to `ubuntu-latest`.
3.  **Strict Lockfile Policy**: `lockfile-guard.yml` prevents lockfile changes in PRs without process.
4.  **Suspicious Actions**: `summary.yml` uses an AI inference action that warrants investigation.
5.  **Disabled Workflows**: `gs-agent`, `gs-control`, and `gs-gateway` deployments are explicitly disabled during Phase 0 to isolate CI for core services.

## Next Steps (Phase 1)

*   Delete or repair `jules-nightly.yml`.
*   Investigate `summary.yml`.
*   Review `lockfile-guard.yml` strictness.
