...
async function ensurePagesOutputDirRule() {
  ...
  const pkgRes: any = await gh.rest.repos
    .getContent({ owner, repo, path: 'package.json' })
    .catch(() => null);
  if (!pkgRes || Array.isArray(pkgRes.data)) return;
  const pkg = JSON.parse(
    Buffer.from((pkgRes.data as any).content, 'base64').toString('utf8'),
  );
  ...
  for (const check of cfg.cloudflare.checks as any[]) {
    if (check.type === 'pages_build_status') {
      ...
      if (!['success', 'completed'].includes(status)) {
        await openOpsIssue(
          ...
          `Latest build stage status: \`${status}\`. Trigger rebuild or inspect Pages logs.`,
    ...
    if (check.type === 'dns_records') {
      ...
      for (const req of check.required) {
        const hit = (dns as any[]).find(
          (d: any) =>
            d.name === req.name &&
            d.type === req.type &&
            (req.contains
              ? String(d.content || '').includes(req.contains)
              : true),
        );
        if (!hit) {
          await openOpsIssue(
