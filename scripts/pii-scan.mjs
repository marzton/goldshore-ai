...
const outputPath = getArgValue(
  '--output',
  'apps/admin/src/data/pii-scan-results.json',
);
...
const webRoots = ['apps/web', 'apps/admin', 'public', 'docs'];
...
const buildSummary = (findings) => {
  ...
  const status =
    counts.high > 0 ? 'Critical' : counts.medium > 0 ? 'Warning' : 'Clear';
...
const main = async () => {
  ...
  for (const file of files) {
    ...
    matches.forEach((match) => {
      ...
      targetArray.push({
        id: `PII-${String(codeFindings.length + webFindings.length + 1).padStart(3, '0')}`,
        ...match,
      });
    });
  }
  ...
  const alertStatus =
    summary.high > 0 ? 'Immediate' : summary.total > 0 ? 'Monitor' : 'Clear';
  const report = {
    ...
    sources: [
      {
        ...
        scanType: 'code',
        scannedFiles: scannedCode,
        findings: codeFindings.length,
      },
      {
        name: 'Web content',
        scanType: 'web',
        scannedFiles: scannedWeb,
        findings: webFindings.length,
      },
    ],
    findings,
    remediation,
  ...
  if (mode === 'ci' && summary.total > 0) {
    console.log(
      `::warning::PII scan found ${summary.total} potential matches (${summary.high} high severity).`,
    );
  }
  if (mode === 'ci' && summary.high > 0) {
    console.log(
      '::warning::High severity PII detected. Immediate remediation recommended.',
    );
  }

  if (mode === 'pre-commit' && summary.total > 0) {
    console.log(
      `PII scan warning: ${summary.total} potential matches found (${summary.high} high severity).`,
    );
    console.log(
      'Review findings and follow remediation guidance before commit.',
    );
  }
