...
## 12) Merge-able Branch Audit (GoldShore Standard)

Use the repo script to quickly scan remote branches for clean mergeability into a target branch.

```bash
pnpm audit:mergeable
# or custom target
bash scripts/merge-audit.sh origin/staging
```

What it does:

- fetches/prunes remotes (can disable with `MERGE_AUDIT_FETCH=0`)
- checks each `origin/*` branch against target using a dry-run merge-tree strategy
- prints `mergeable` vs `conflicts` plus ahead/behind counts

Typical workflow:

1. Keep `main` deployable
2. Merge clean branches first
3. Rebase conflict branches on target and rerun audit
